...
const (
	...
	userAgent      = "go-minox/" + libraryVersion
...
func (c *Client) GetEndpointURL(relative string, pathParams PathParams) url.URL {
	...
	clientURL.Path = strings.Replace(clientURL.Path, "{{.tenant_id}}", strconv.Itoa(c.TenantID()), -1)
	// clientURL.RawQuery = strings.Replace(clientURL.RawQuery, "{{.tenant_id}}", strconv.Itoa(c.TenantID()), -1)
